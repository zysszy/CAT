...
def tf_cos_d_sim(x, y):
    ...
    #print ("tf_cos")
    #print (x, y)
    if len(x[0]) == len(y[0]) and len(x[0]) == 0 and len(x) == len(y) and len(x) == 1:
        return 0
    #if len(x[0]) == len(y[0]) and len(x[0]) == 0:
    #    return 0
    for i in range(len(x)):
        x_now = x[i]
        y_now = y[i]
...
def doone(met, prob):
    f = open(f"Com_Mutated_{met}.{prob}en", "r")
    en_new_lines = f.readlines()
    f.close()

    f = open(f"Com_Original_{met}.{prob}en", "r")
    en_old_lines = f.readlines()
    f.close()

    f = open(f"Com_Mutated_{met}.{prob}zh", "r")
    new_lines = f.readlines()
    f.close()

    f = open(f"Com_Original_{met}.{prob}zh", "r")
    old_lines = f.readlines()
    f.close()

    f = open(f"Com_oracle_{met}.{prob}zh", "r")
    ...
    write_all(lcs_l, ed_l, tfidf_l, bleu_l, lcs_d, ed_d, tfidf_d, f"Com_ALL_{met}.{prob}txt")
...
mett = ["LCS", "ED", "TFIDF", "BLEU"]
probb = ["", "prob."]
[doone(m, p) for m in mett for p in probb]
